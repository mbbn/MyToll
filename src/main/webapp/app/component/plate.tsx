...
const Plate = (props: IPlateProps) => {
  const {plateCode, editable = true, error, helperText} = props;

  const myTheme: Theme = myTollTheme;
  const [bColor, setBColor] = useState(!error ? myTheme.palette.grey["500"] : myTheme.palette.error.main);

  const [part1, setPart1] = useState(plateCode ? plateCode.substr(0, 2) : null);
  const [part2, setPart2] = useState(plateCode ? plateCode.substr(2, 2) : null);
  const [part3, setPart3] = useState(plateCode ? plateCode.substr(4, 3) : null);
  const [part4, setPart4] = useState(plateCode ? plateCode.substr(7, 2) : null);
  ...
  return (<FormControl>
    <Grid container spacing={0} justify="center"
      ...
      <Grid item xs={2}>
        <TextField name={'part1'} value={part1} disabled={!editable} error={error} autoComplete={'off'} onChange={event => {
          OnChange(event, "part1")
        }} onFocus={OnFocused} onBlur={OnBlur} style={{margin: 0}}
                   variant={"standard"}
                   InputProps={{inputProps: {style: {textAlign: 'center'}}, disableUnderline: true}}
                   maxLength={2}/>
      </Grid>
      <Grid item xs={3}>
        {editable ? <Select name={'part2'} value={part2} error={error} style={{margin: 0}} onChange={event => {
      ...
      <Grid item xs={3} style={{borderLeftStyle: "solid", borderColor: bColor}}>
        ...
        <TextField name={'part4'} value={part4} disabled={!editable} autoComplete={'off'} error={error} style={{margin: 0}} onChange={event => {
